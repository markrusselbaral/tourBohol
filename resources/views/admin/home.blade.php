      ...
      <div class="modal-dialog" role="document">
         <div class="modal-content">
            <div class="modal-header">
               <h6 class="modal-title m-0" id="exampleModalDefaultLabel">Add Home Content</h6>
                  ...
                  <div class="card">
                     <div class="card-body">
                        <div class="mb-3">
                           ...
                           <input type="text" class="form-control" id="textContent" placeholder="Enter text" name="text">
                        </div>
                        <div class="mb-3">
...
<form action="{{ route('home.update') }}" method="POST" enctype="multipart/form-data">
   ...
   <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalDefaultLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
         <div class="modal-content">
            <div class="modal-header">
               <h6 class="modal-title m-0" id="exampleModalDefaultLabel">Edit Home Content</h6>
               <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <!--end modal-header-->
            <div class="modal-body">
                  ...
                  <div class="card">
                     <div class="card-body">
                        <div class="mb-3">
                           ...
                           <input type="text" class="form-control" id="editText" placeholder="Enter text" name="text">
                        </div>
                        <div class="mb-3">
            ...
            <div class="col-12">
               <div class="card">
                  <div class="card-header">
                     <h4 class="card-title">Homepage Contents</h4>
                  </div>
                  <!--end card-header-->
                  <div class="card-body">
                        ...
                        <table class="table" id="datatable_1">
                           ...
                           <tbody>
                              ...
                              <tr>
                                 <td>{{ $contents->text }}</td>
                                 <td>
                                    <img src="{{ asset('storage/home/'.$contents->background_image) }}" width="50px" height="50px">
                                 </td>
                                 <td>
                                    <form action="{{ route('home.select') }}" method="POST">
                                       @csrf
                                       @method('PATCH')
                                       <input type="hidden" value="{{ $contents->id }}" name="id">
                                       <button type="submit" class="btn btn-{{ $contents->status == 1 ? 'success' : 'info' }}">{{ $contents->status == 1 ? 'Selected' : 'Select' }}</button>
                                    </form>
                                 </td>
                                 <td>
                                    <form action="{{ route('home.delete') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete?')">
                                       @csrf
                                       @method('DELETE')
                                       <input type="hidden" value="{{ $contents->id }}" name="id">
    ...
    $(document).on('click', '.editbtn', function () {
        ...
        $.ajax({
           ...
           success: function (response) {
                ...
                $('#currentImage').attr('src', '{{ asset('storage/home/') }}' + '/' + response.background_image);
                $('#imageFilename').html(response.background_image);
